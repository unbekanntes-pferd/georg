...
pub use models::{
    AssistantResponse, CandidateRequests, CandidateResponse, ChildCareRequestResponse,
};
...
#[tauri::command]
pub async fn import_candidate_data(
    path: String,
    state: tauri::State<'_, AppState>,
) -> Result<(), String> {
    let path = Path::new(&path).to_owned(); // Ensure path is owned before moving into the thread

    let import_files = get_import_files(&path).map_err(|err| err.to_string())?;
...
#[tauri::command]
pub async fn import_assistant_data(
    path: String,
    state: tauri::State<'_, AppState>,
) -> Result<(), String> {
    let path = Path::new(&path).to_owned(); // Ensure path is owned before moving into the thread
    ...
    let assistant_data = parse_assistant_data(&import_files).map_err(|err| err.to_string())?;

    state.inner().inner().update_assistant_data(assistant_data);
...
fn get_import_files(path: &Path) -> Result<ImportFiles, GeorgError> {
    ...
    let candidates_file =
        find_file(&xlsx_files, CANDIDATE_XLSX_NAME).ok_or(GeorgError::MissingCandidatesFile)?;

    let assistants_file =
        find_file(&xlsx_files, ASSISTANTS_XLSX_NAME).ok_or(GeorgError::MissingAssistantsFile)?;
