...
<div class="p-4">
	{#if schoolAssistantMatches}
		{#each schoolAssistantMatches as schoolAssistantMatch}
			<div
				class="border rounded-md p-2 mb-2 grid grid-cols-2 gap-4 assistantCard first:bg-surface-700 dark:first:bg-surface-200"
			>
				...
				<div class="flex flex-row">
					...
					<p class="location">
						{schoolAssistantMatch.assistant.city ? schoolAssistantMatch.assistant.city : '-'} (Distanz:
						{schoolAssistantMatch ? schoolAssistantMatch.distance.toFixed(0) : '-'} km)
					</p>
				</div>
				<div class="flex flex-row">
					...
					<Child class="mx-2" />
					<p>
						{schoolAssistantMatch.assistant.assignedChild
							? schoolAssistantMatch.assistant.assignedChild
							: '-'}
					</p>
				</div>
				...
				<div class="flex flex-row">
					...
					<Approved class="mx-2" />
					<p>
						{schoolAssistantMatch.assistant.approved
							? schoolAssistantMatch.assistant.approved
							: '-'}
					</p>
				</div>
				...
				<div class="flex flex-row">
					...
					<p>
						{schoolAssistantMatch.assistant.certifications
							? schoolAssistantMatch.assistant.certifications
							: '-'}
					</p>
				</div>
				<div class="flex flex-row">
					...
					<Family class="mx-2" />
					<p>
						{schoolAssistantMatch.assistant.children
							? schoolAssistantMatch.assistant.children
							: '-'}
					</p>
				</div>
...
<style>
	...
	.assistantCard:first-child .location {
